...
const WA_NUMBER = '628157704441'; // +62 815-7704-441
...
// ========== Render Featured Products (index) ==========
    ...
    <div class="product-card glass-card overflow-hidden">
      ...
      <div class="p-5">
        ...
        <div class="flex items-baseline gap-2 mb-4">
          <span class="text-xl font-extrabold gradient-text">${formatRupiah(p.priceDaily)}</span>
          <span class="text-zinc-500 text-sm">/hari</span>
        </div>
        <button onclick="sewaWhatsApp('${p.name}', ${p.priceDaily})" class="btn-energy w-full text-center text-sm">
          Sewa via WhatsApp
        </button>
      </div>
    </div>
...
// ========== Render All Products (produk.html) ==========
    ...
    <div class="product-card glass-card overflow-hidden" data-category="${p.category}">
      ...
      <div class="p-5">
        ...
        <div class="space-y-1 mb-4 text-sm">
          <div class="flex justify-between"><span class="text-zinc-500">Harian</span><span class="font-semibold">${formatRupiah(p.priceDaily)}</span></div>
          <div class="flex justify-between"><span class="text-zinc-500">Mingguan</span><span class="font-semibold">${formatRupiah(p.priceWeekly)}</span></div>
          <div class="flex justify-between"><span class="text-zinc-500">Bulanan</span><span class="font-semibold text-accent">${formatRupiah(p.priceMonthly)}</span></div>
        </div>
        <button onclick="sewaWhatsApp('${p.name}', ${p.priceDaily})" class="btn-energy w-full text-center text-sm" ${p.stock !== 'tersedia' ? 'disabled style="opacity:0.5;cursor:not-allowed"' : ''}>
...
// ========== Sewa via WhatsApp (one-click) ==========
function sewaWhatsApp(namaAlat, hargaHarian) {
  const text = `Halo Griya Fitnes, saya ingin sewa di persewaanalatfitnes.com:
• Alat: ${namaAlat}
• Harga: ${formatRupiah(hargaHarian)}/hari
• Durasi: [isi]
• Kota: [Semarang / Solo / Jogja]
• Nama: [isi]
Mohon info ketersediaan & cara booking. Terima kasih!`;
...
async function renderReviews() {
  ...
  container.innerHTML = reviews.map(r => `
    <div class="glass-card p-6 flex flex-col">
      ...
      <p class="text-zinc-300 text-sm leading-relaxed mb-4 flex-1">"${r.text}"</p>
      <div class="flex items-center gap-3">
        <div class="w-10 h-10 rounded-full bg-lime-500/20 flex items-center justify-center text-lime-400 font-bold text-sm">
          ${r.name.charAt(0)}
        </div>
        <div>
          <p class="font-semibold text-sm">${r.name}</p>
          <p class="text-zinc-500 text-xs">${r.city}</p>
        </div>
      </div>
    </div>
  `).join('');
...
// ========== FAQ Accordion (native details works, extra polish) ==========
function initFAQ() {
  // Native <details> is sufficient; optional enhancement if needed
}
...
async function populateProductSelect() {
  ...
  products.forEach(p => {
    ...
    opt.value = p.name;
    opt.textContent = `${p.name} — ${formatRupiah(p.priceDaily)}/hari`;
